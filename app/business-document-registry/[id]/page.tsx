...
export default function BusinessDocumentDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleView = (fileUrl?: string) => {
    const urlToUse = fileUrl || document?.file_url;
    if (urlToUse) {
  ...
  const availableVersions = getAvailableVersions(document?.fileVersions, 'document_version');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-brand-gray2/50 rounded-lg border border-brand-gray1 p-6">
        ...
        {(document.file_name || (document.fileVersions && document.fileVersions.length > 0)) && (
          <div className="mt-6">
            <div className="flex items-center justify-between mb-2">
              <label className="block text-sm font-medium text-brand-gray3">
                Attached Files
              </label>
              {(document.fileVersions && document.fileVersions.length > 0) && (
                <div className="flex items-center gap-2">
                  ...
                  >
                    <option value="current">Current ({document.version || 'N/A'})</option>
