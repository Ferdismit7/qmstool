...
export default function BusinessImprovementDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleDownload = (fileUrl?: string) => {
    const urlToUse = fileUrl || improvement?.file_url;
    if (urlToUse) {
  ...
  const availableVersions = getAvailableVersions(improvement?.fileVersions, 'improvement_version');

  // Get files to display based on selected version
  const filesToDisplay = getFilesToDisplay(
    selectedFileVersion,
    improvement ? {
      file_url: improvement.file_url,
      file_name: improvement.file_name,
      file_size: improvement.file_size,
      file_type: improvement.file_type
    } : null,
    improvement?.version,
    improvement?.fileVersions,
    'improvement_version'
  );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              {(improvement.file_name || (improvement.fileVersions && improvement.fileVersions.length > 0)) && (
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-sm font-medium text-brand-gray3">Attached Files</label>
                    {(improvement.fileVersions && improvement.fileVersions.length > 0) && (
                      <div className="flex items-center gap-2">
                        ...
                        >
                          <option value="current">Current ({improvement.version || 'N/A'})</option>
