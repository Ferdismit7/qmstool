...
export default function RecordKeepingSystemDetail({ params }: { params: Promise<{ id: string }> }) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 401) {
            ...
            if (isMounted) {
              setError('You do not have permission to view this record keeping system. Please ensure you have access to the system\'s business area.');
            }
            return;
          } else if (response.status === 404) {
            if (isMounted) {
              setError('Record keeping system not found. It may have been deleted or you do not have access to it.');
            }
            return;
          } else {
            const errorData = await response.json().catch(() => ({}));
            if (isMounted) {
              setError(errorData.error || 'Failed to fetch record keeping system');
            }
            return;
          }
        }
        ...
        if (data.success && isMounted) {
          ...
          setError(data.error || 'Failed to fetch record keeping system');
        }
      } catch (error) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch('/api/record-keeping-systems/soft-delete', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to delete record keeping system');
      }
      ...
      setNotification({
        ...
        type: 'success',
        title: 'Success',
        message: 'Record keeping system successfully deleted'
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <DeleteConfirmationModal
        ...
        itemName={system?.system_name || `System ${system?.id}` || ''}
